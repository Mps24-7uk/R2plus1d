...
def _imread_unicode(p: str):
    """Robust imread (Unicode/long paths) using np.fromfile + cv2.imdecode."""
    try:
...
def load_clip_from_folder_cv2(folder, seq_len=12, height=224, width=224,
    ...
    frames = []
    for p in paths:
        img_bgr = _imread_unicode(p)              # BGR, uint8
        if img_bgr is None:
            raise ValueError(f"Failed to read image: {p}")
        img_rgb = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB)
        img_rgb = cv2.resize(img_rgb, (width, height), interpolation=cv2.INTER_LINEAR)  # bilinear
...
# --------------------- main ---------------------
def main():
    ap = argparse.ArgumentParser(description="TensorRT inference (OpenCV preprocessing)")
    ap.add_argument("--engine", required=True, help="Path to best.engine")
    ap.add_argument("--input",  required=True, help="Dir of frames OR root dir with many clip folders")
    ap.add_argument("--seq-len", type=int, default=12)
    ap.add_argument("--height",  type=int, default=224)
    ap.add_argument("--width",   type=int, default=224)
    ap.add_argument("--save-npy", action="store_true", help="Save first clip tensor as clip.npy for parity check")
    args = ap.parse_args()
    ...
    for i, d in enumerate(seq_dirs):
        try:
            clip = load_clip_from_folder_cv2(d, args.seq_len, args.height, args.width)
