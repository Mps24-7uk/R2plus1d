...
def main():
    parser = argparse.ArgumentParser(description="Export R(2+1)D-18 .pth to ONNX (grayscale).")
    ...
    parser.add_argument("--image-size", type=int, default=224, help="Square input size H=W (default: 224)")
    parser.add_argument("--opset", type=int, default=17, help="ONNX opset version (default: 17)")
    # in case you ever want RGB later, you can change this to 3 on CLI
    parser.add_argument("--in-channels", type=int, default=1, help="Number of input channels (1=grayscale)")
    ...
    # Output path = same directory as checkpoint, named best.onnx
    ...
    print(f"[INFO] Using in_channels={args.in_channels} for grayscale export")

    # 🔸 IMPORTANT: set in_channels to 1 for grayscale model
    model = R2Plus1DModel(
    ...
    b = args.batch_size
    c = args.in_channels         # 1 for grayscale
    t = args.num_frames
    h = w = args.image_size
    ...
    output_names = ["logits"]
    dynamic_axes = {
        "input":  {0: "batch", 2: "time"},  # variable batch and time
        "logits": {0: "batch"},
    }
